      ...
      <div className="py-20">
        <div className="flex flex-col justify-center items-center">
          <div className="relative">
            ...
            <div className="animate-spin rounded-full h-10 w-10 border-4 border-gray-900 border-t-transparent absolute top-0 left-0"></div>
          </div>
    ...
    return (
      <div className="p-6 border border-gray-200 rounded-xl bg-white">
        <div className="flex items-center">
          ...
          <div className="ml-3">
            <p className="text-gray-900 font-medium">Error loading summary</p>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="border border-gray-200 rounded-xl p-4 bg-white">
          ...
          <p className="text-xs text-gray-500 mt-1">{getMonthName(selectedMonth.month)} {selectedMonth.year}</p>
        </div>
        <div className="border border-gray-200 rounded-xl p-4 bg-white">
          <p className="text-sm text-gray-600">Daily Average</p>
          <p className="text-2xl font-semibold text-gray-900 mt-1">₹{getAverageDaily().toFixed(2)}</p>
          <p className="text-xs text-gray-500 mt-1">{summary.dailyBreakdown.length} active days</p>
        </div>
        <div className="border border-gray-200 rounded-xl p-4 bg-white">
        ...
        <div className="border border-gray-200 rounded-xl p-4 bg-white">
          ...
          <p className="text-xs text-gray-500 mt-1">{summary.topExpenses.length} total expenses</p>
        </div>
      </div>
      ...
      <div className="border border-gray-200 rounded-xl p-6 bg-white">
        ...
        {summary.dailyBreakdown.length === 0 ? (
          <div className="text-center text-sm text-gray-600">No expenses recorded for this month.</div>
        ) : (
            ...
            {summary.dailyBreakdown.map((day, i) => {
              ...
              return (
                <div key={i}>
                  ...
                  <p className="text-xs text-gray-500 mt-1">{percentage.toFixed(1)}% of monthly total</p>
                </div>
              );
            })}
          </div>
